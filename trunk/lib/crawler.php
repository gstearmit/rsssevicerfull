...
{
	static $items = array();
public static function save_site_structure($cmd,$id,$site){
...
public static	function save_site($cmd,$id){
...
public static	function get_pattern($site_id)
	{
		return DB::fetch_all('
			SELECT
				*
...
public static	function format_link($source,$format=false)
	{
		if($format)
		{
...
public static	function save($sour,$dest)
	{
...
public static	function parse_row($link,$pattern,$site)
	{
...
public static	function get_data($site,$pattern)
	{
		...
		if($html){
			...
			foreach($html->find($pattern_bound) as $item)
			{
				...
				foreach($item->find($pattern_link) as $link){
					$link2 = Feed::check_link($link->getAttribute('href'),$host);
				}
				if(Feed::check_url($link2)){
					...
					//Feed::debug($site);
					Feed::parse_row($link2,$pattern,$site);
				}
			}
		}
		$html->clear();
...
public static	function check_link($url,$host='')
	{
...
public static	function check_url($url=NULL){
...
public static	function html_no_comment($url) {
		// create HTML DOM
