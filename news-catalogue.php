...
$id=(isset($_REQUEST['id']) and $_REQUEST['id'])?$_REQUEST['id']:1;
$category_id=(isset($_REQUEST['category']) and $_REQUEST['category'])?$_REQUEST['category']: Null;
$total = DB::fetch('select count(*) as total from news where news.category_id='.$category_id,'total');
$items = DB::fetch_all('select * from news where news.category_id='.$category_id.' ORDER BY id DESC limit '.(($page_no-1)*$item_per_page).','.$item_per_page);
